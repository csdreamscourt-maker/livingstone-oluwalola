...
export function HomeClient({ frameworks, companies, articles, heroTitle, heroDescription }: HomeClientProps) {
  const stats = [
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {companies.slice(0, 4).map((company, index) => (
              <motion.div
                ...
                whileHover={{ y: -4 }}
              >
                <Link href={`/companies/${company.slug}`} className="group flex h-full flex-col gap-3 rounded-xl bg-white p-6 shadow-sm transition-shadow duration-200 hover:shadow-xl">
