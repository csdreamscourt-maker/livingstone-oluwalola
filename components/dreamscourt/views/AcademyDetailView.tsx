...
export function AcademyDetailView({ slug }: { slug: string }) {
  ...
  if (notFound || !course) {
    return (
      <GlassCard>
        <p className="text-sm text-white/60">Course not found.</p>
        <Link href="/academy" className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-gold-300">
  ...
  return (
    <div className="space-y-5">
      <Link href="/academy" className="inline-flex items-center gap-1.5 text-sm font-semibold text-white/50 hover:text-white">
        <ArrowLeft className="h-3.5 w-3.5" />
        Back to Academy
      </Link>
      ...
      <GlassCard>
        ...
        {course.description && <p className="mt-3 max-w-2xl text-sm leading-7 text-white/60">{course.description}</p>}
        ...
        {course.selar_url ? (
          ...
          <p className="mt-6 text-sm text-white/40">Enrollment link coming soon.</p>
        )}
      </GlassCard>
    </div>
  );
